...
export default function RillaUsageDashboardApp() {
  ...
  const selectedStats = useMemo(() => {
    ...
    for (const c of conversations) {
      ...
      const existing = groupedMap.get(key) || { key, label: granularity === "day" ? shortDate(bucketDate) : granularity === "month" ? bucketDate.toLocaleDateString(undefined, { month: "short" }) : `Wk ${shortDate(bucketDate)}`, conversations: 0, sold: 0, totalDuration: 0, talkSum: 0, talkCount: 0 };
    ...
    const trend = Array.from(groupedMap.values()).sort((a, b) => new Date(a.key) - new Date(b.key)).map((item) => ({ ...item, avgTalkRatio: item.talkCount ? item.talkSum / item.talkCount : null, avgDurationMinutes: item.conversations ? Math.round(item.totalDuration / item.conversations / 60) : 0 }));
      ...
      <div className="relative mx-auto max-w-[1800px] space-y-5">
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.65fr_1fr]">
          <Section title="Conversation Trend" subtitle={`${periodLabel}. Date filters use the conversation-detail sheet.`} right={<div className="text-sm text-slate-400">{fmtInt(selectedStats.conversations)} conversations · {fmtMoney(selectedStats.totalSold)} sold</div>}>
            <div className="h-[340px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={selectedStats.trend} margin={{ top: 8, right: 20, left: 0, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke={chartGrid} /><XAxis dataKey="label" stroke={chartAxis} tick={{ fontSize: 12 }} /><YAxis yAxisId="left" stroke={chartAxis} tick={{ fontSize: 12 }} /><YAxis yAxisId="right" orientation="right" tickFormatter={(v) => `${Math.round(v * 100)}%`} stroke={chartAxis} tick={{ fontSize: 12 }} /><Tooltip contentStyle={{ background: tooltipBg, border: `1px solid ${tooltipBorder}`, borderRadius: 14 }} labelStyle={{ color: tooltipLabel }} formatter={(value, name) => (name === "avgTalkRatio" ? fmtPct(value, 1) : value?.toLocaleString?.() || value)} /><Legend /><Line yAxisId="left" type="monotone" dataKey="conversations" name="Conversations" stroke="#2563eb" strokeWidth={3} dot={false} /><Line yAxisId="right" type="monotone" dataKey="avgTalkRatio" name="Avg Talk Ratio" stroke="#f59e0b" strokeWidth={3} dot={false} /></LineChart></ResponsiveContainer></div>
          </Section>
